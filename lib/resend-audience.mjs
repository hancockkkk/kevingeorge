...
// Resend reads return { value, type } for each custom property.
export const contactProperties = contact => Object.fromEntries(
  Object.entries(contact?.properties ?? {}).map(([key, property]) => [key, property?.value ?? property]),
);
...
export async function saveSubscriber(email) {
  ...
  const previous = contactProperties(existing);
...
export async function migrateSubscriber(row) {
  ...
  if (!saved?.properties || Object.entries(properties).some(([key, value]) => contactProperties(saved)[key] !== value)) {
